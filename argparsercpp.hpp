...
private:
    ...
    struct Argument {
        ...
        Argument(const String &_short_name, const String &_name, const String &_description, bool _optional, char nargs)
            : short_name(_short_name), name(_name), description(_description), optional(_optional)
        {
            if (nargs == '+' || nargs == '*')
            {
...
public:
    ...
    {
        Argument arg("-h", "--help", "show this help message", true, 0);
        insertArgument(arg);
    }
    ...
    void addArgument(const String &name, const String &_description, char nargs = 0, bool optional = true)
    {
        if (name.size() > 2)
        {
            Argument arg("", verify(name), _description, optional, nargs);
            insertArgument(arg);
        }
        else
        {
    ...
    void addArgument(const String &short_name, const String &name, const String &_description, char nargs = 0, bool optional = true)
    {
        Argument arg(verify(short_name), verify(name), _description, optional, nargs);
        insertArgument(arg);
    }
    
    void addFinalArgument(const String &name, const String &_description, char nargs = 1, bool optional = false)
    {
        final_name_ = delimit(name);
        Argument arg("", final_name_, _description, optional, nargs);
        insertArgument(arg);
    }
